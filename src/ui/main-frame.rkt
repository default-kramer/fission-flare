...
; Something was making `raco test` lock up.
; Putting everything in the main module solves it.
(module+ main
  ...
  ; Just because we *have* DrRacket doesn't mean we're running *in* DrRacket.
  ; But it's good enough for my purposes, I think.
  (define has-drracket?
    (let ([x (with-handlers ([any/c (lambda (e) #f)])
               (dynamic-require 'drracket/get-module-path #f))])
      (and x #t)))
  (println (list "Has DrRacket?" has-drracket?))
  ...
  (define game-log-port #f) ; #f or a port for the game recording
  (define game-log-dir (getenv "ffreplaydir"))

  (define (cleanup)
    (let* ([log-port game-log-port])
      (when game-log-port
        (close-output-port game-log-port)
        (set! game-log-port #f)))
    (set! controller #f)
    (let ([conn conn])
      (when conn
        ...
        (when (not show?)
          ...
          (ignore-errors
           (when host-frame
             ...
             (set! host-frame #f)))
          ; When we're running the `raco exe` version, using `exit` closes the
          ; console window correctly.
          ; But it's annoying when I'm running from inside DrRacket because it
          ; kills the REPL and jumps me directly to this line.
          (when (not has-drracket?)
            (exit))))
      ))
  ...
  (vn button%
      ...
      [callback
       (lambda args
         (let* ([filters '(("Fission Flare Replays" "*.ffreplay")
                           ("All Files" "*"))]
                [filename (get-file #f frame game-log-dir #f #f null filters)])
           (when filename
